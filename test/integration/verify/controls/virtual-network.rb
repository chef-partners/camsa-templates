...
ama_subnet_prefix = attribute('ama_subnet_prefix', default: '10.3.0.128/25', description: 'The address space assigned to the AMA subnet')
customer_subnet_prefix = attribute('ama_subnet_prefix', default: '10.3.0.0/25', description: 'The address space assigned to the customer subnet')
...
title 'Ensure AMA Subnet exists in customer network'
...
control 'Customer Virtual Network' do
  ...
  describe azure_generic_resource(group_name: customer_resource_group_name, name: customer_virtual_network_name) do
    ...
    its('properties.subnets.count') { should eq 2 }
  end
end

control 'AMA Subnet' do
  impact 1.0
  title 'AMA Subnet has correct address space assigned and has 2 network cards connected to it'

  ama_subnet_name = format('inspec-%s-AMA-Subnet', unique_string)

  subnet = azure_generic_resource(group_name: customer_resource_group_name, name: customer_virtual_network_name)
           .properties.subnets.find { |s| s.name == ama_subnet_name }

  describe subnet do
    its('properties.provisioningState') { should cmp 'Succeeded' }
    its('properties.addressPrefix') { should cmp ama_subnet_prefix }
    its('properties.ipConfigurations.count') { should eq 2 }
  end

  # Ensure the support NIC for Chef is connected
  chef_nic = format('inspec-chef-%s-AMA-NIC', unique_string)

  chef_nic_connected = subnet.properties.ipConfigurations.find { |i| i.id.include? chef_nic }

  describe chef_nic_connected do
    it "NIC #{chef_nic} should be connected to #{ama_subnet_name}" do
      expect(subject).not_to be_nil
    end
  end

  # Ensure the support NIC for Automate is connected
  automate_nic = format('inspec-automate-%s-AMA-NIC', unique_string)
  ...
  describe automate_nic_connected do
    it "NIC #{automate_nic} should be connected to #{ama_subnet_name}" do
      expect(subject).not_to be_nil
    end
  end  
end
